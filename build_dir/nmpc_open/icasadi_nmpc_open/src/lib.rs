...
const NUM_STATIC_PARAMETERS: usize = 218;

/// Number of decision variables
const NUM_DECISION_VARIABLES: usize = 140;

/// Number of ALM-type constraints (dimension of F1, i.e., n1)
const NUM_CONSTRAINTS_TYPE_ALM: usize = 140;
...
extern "C" {
    fn init_interface_nmpc_open();
    fn cost_function_nmpc_open(
        arg: *const *const c_double,
        casadi_results: *mut *mut c_double) -> c_int;
    fn grad_cost_function_nmpc_open(
        arg: *const *const c_double,
        casadi_results: *mut *mut c_double)
        -> c_int;
    fn mapping_f1_function_nmpc_open(
        arg: *const *const c_double,
        casadi_results: *mut *mut c_double,
    ) -> c_int;
    fn mapping_f2_function_nmpc_open(
        arg: *const *const c_double,
        casadi_results: *mut *mut c_double,
    ) -> c_int;
    // Compute all preconditioning parameters
    fn preconditioning_www_nmpc_open (
        arg: *const *const c_double,
    ) -> c_int;
    fn init_penalty_function_nmpc_open(
        arg: *const *const c_double,
        ipnlt: *mut *mut c_double,
    ) -> c_int;
    fn get_w_cost_nmpc_open() -> c_double;
} // END of extern C
...
pub fn precondition(
    ...
    let arguments = &[u.as_ptr(), static_params.as_ptr()];
    unsafe {
         preconditioning_www_nmpc_open(arguments.as_ptr()) as i32
    }
}

/// Computes the initial penalty
///
/// Make sure that you have called init_nmpc_open
pub fn initial_penalty(
    u: &[f64],
    static_params: &[f64],
    rho_init: &mut f64,) -> i32 {

    // preconditioning_init_penalty_function_nmpc_open
    let arguments = &[u.as_ptr(), static_params.as_ptr()];
    let ip = &mut [rho_init as *mut c_double];

    unsafe {
         init_penalty_function_nmpc_open(
            arguments.as_ptr(),
            ip.as_mut_ptr(),
        ) as i32
    }
